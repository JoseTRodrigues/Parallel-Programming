...
if rank==0:
    ...
    displ_list=[]
    for w in range(nl): 
        displ = [(i+(final-1)*w)*(np+1) for i in range(1,numProc+1)]
...
for m in range(1,4):
    ...
    if m < final: #cada processador aplica a eliminação de gauss a nl linhas
        ...
        for i in range(nl):
            ...
            flush_print(i,f'rank{rank} received data:',Ap)
